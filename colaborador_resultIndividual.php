...
<script type="text/javascript">
    ...
    Highcharts.chart('container', {
        ...
        series: [
            ...
            // GRAFIAS PARA PERIODO SEMANAL
            if ($periodoInd_v == "7ind_v") {
                $consultaProspectos = "SELECT COUNT(*) FROM folios WHERE t_solicitud = 'ALTA DE POLIZA' AND fecha BETWEEN '$periodoSemInd1_v' AND '$periodoSemInd2_v' AND id_agente = '$id_agente'";
                $resultProspectos = mysqli_query($conexion, $consultaProspectos);
                $prospectos = mysqli_fetch_array($resultProspectos);
                $datosProspectos = $prospectos[0];
            }
            ...
            if ($periodoInd_v == "7ind_v") {
                $consultaCitas = "SELECT COUNT(*) FROM folios WHERE t_solicitud = 'MOVIMIENTOS' AND fecha BETWEEN '$periodoSemInd1_v' AND '$periodoSemInd2_v' AND id_agente = '$id_agente'";
                $resultCitas = mysqli_query($conexion, $consultaCitas);
                $citas = mysqli_fetch_array($resultCitas);
                $datosCitas = $citas[0];
            }
            ...
            if ($periodoInd_v == "7ind_v") {
                $consultaEntrevista = "SELECT COUNT(*) FROM folios WHERE t_solicitud = 'PAGOS' AND fecha BETWEEN '$periodoSemInd1_v' AND '$periodoSemInd2_v' AND id_agente = '$id_agente'";
                $resultEntrevista = mysqli_query($conexion, $consultaEntrevista);
                $entrevista = mysqli_fetch_array($resultEntrevista);
                $datosEntrevista = $entrevista[0];
            }
