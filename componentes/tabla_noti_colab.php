...
<div>
    ...
    <table id="tablaDinamicaLoad" class="table table-striped table-bordered" style="width:100%">
        ...
        <tbody>
            ...
            <?php
            $sqlTipo = "SELECT tipo FROM notificaciones WHERE contador = '1' AND folio > '0' GROUP BY tipo";
            ...
            ?>
                    <tr>
                        ...
                        $sqlTramite = "SELECT folio FROM notificaciones WHERE contador = '1' AND tipo = 'TRAMITE' AND folio > '0' GROUP BY folio DESC";
                        $resultTramite = mysqli_query($conexion, $sqlTramite);
                        while ($verTramite = mysqli_fetch_array($resultTramite)) {
                            $datoTramite = $verTramite[0];

                            $sqlUsuario = "SELECT usuario FROM notificaciones WHERE contador = '1' AND (tipo = 'TRAMITE' AND folio = '$datoTramite') GROUP BY folio";
                            $resultUsuario = mysqli_query($conexion, $sqlUsuario);
                            while ($verUsuario = mysqli_fetch_array($resultUsuario)) {
                                $datoUsuario = $verUsuario[0];
                            }

                            $sqlFecha = "SELECT fecha FROM notificaciones WHERE contador = '1' AND (tipo = 'TRAMITE' AND folio = '$datoTramite')";
                        ...
                        ?>
                            ...
                            <!-- Detalles -->
                            <td align="center">
                                <form class="" action="seguimiento_colab.php?id=<?php echo $datoTramite; ?>" method="post">
                ...
                <tr>
                    ...
                    <?php
                        $sqlComent = "SELECT folio FROM notificaciones WHERE contador = '1' AND tipo = 'COMENTARIO' AND folio > '0' GROUP BY folio DESC";
                        $resultComent = mysqli_query($conexion, $sqlComent);
                        while ($verComent = mysqli_fetch_array($resultComent)) {
                            $datoComent = $verComent[0];

                            $sqlUsuario = "SELECT usuario FROM notificaciones WHERE contador = '1' AND (tipo = 'COMENTARIO' AND folio = '$datoComent') GROUP BY folio";
                            $resultUsuario = mysqli_query($conexion, $sqlUsuario);
                            while ($verUsuario = mysqli_fetch_array($resultUsuario)) {
                                $datoUsuario = $verUsuario[0];
                            }

                            $sqlConteoC = "SELECT COUNT(*) FROM notificaciones WHERE contador = '1' AND (tipo = 'COMENTARIO' AND folio = '$datoComent') GROUP BY folio";
                            $resultConteoC = mysqli_query($conexion, $sqlConteoC);
                            while ($verConteoC = mysqli_fetch_array($resultConteoC)) {
                                $datoConteoC = $verConteoC[0];
                            }

                            $sqlFultima = "SELECT MAX(fecha) AS id FROM notificaciones WHERE contador = '1' AND (tipo = 'COMENTARIO' AND folio = '$datoComent') ";
                    ...
                    ?>
                        ...
                        <!-- Detalles -->
                        <td align="center">
                            <form class="" action="seguimiento_colab.php?id=<?php echo $datoComent; ?>" method="post">
            ...
            <tr>
                ...
                <?php
                        $sqlAr = "SELECT folio FROM notificaciones WHERE contador = '1' AND tipo = 'ARCHIVO' AND folio > '0' GROUP BY folio DESC";
                        $resultAr = mysqli_query($conexion, $sqlAr);
                        while ($verAr = mysqli_fetch_array($resultAr)) {
                            $datoAr = $verAr[0];

                            $sqlUsuario = "SELECT usuario FROM notificaciones WHERE contador = '1' AND (tipo = 'ARCHIVO' AND folio = '$datoAr') GROUP BY folio";
                            $resultUsuario = mysqli_query($conexion, $sqlUsuario);
                            while ($verUsuario = mysqli_fetch_array($resultUsuario)) {
                                $datoUsuario = $verUsuario[0];
                            }

                            $sqlConteoA = "SELECT COUNT(*) FROM notificaciones WHERE contador = '1' AND (tipo = 'ARCHIVO' AND folio = '$datoAr') GROUP BY folio";
                            $resultConteoA = mysqli_query($conexion, $sqlConteoA);
                            while ($verConteoA = mysqli_fetch_array($resultConteoA)) {
                                $datoConteoA = $verConteoA[0];
                            }

                            $sqlFultima = "SELECT MAX(fecha) AS id FROM notificaciones WHERE contador = '1' AND (tipo = 'ARCHIVO' AND folio = '$datoAr')";
                            $resultFultima = mysqli_query($conexion, $sqlFultima);
                            while ($verFultima = mysqli_fetch_array($resultFultima)) {
                                $datoFultima = $verFultima[0];
                ...
                ?>
                    ...
                    <!-- Detalles -->
                    <td align="center">
                        <form class="" action="seguimiento_colab.php?id=<?php echo $datoAr; ?>" method="post">
        ...
        <tr>
            <!-- Querys para la seccion de reactivaciones -->
            <?php
                        $sqlReact = "SELECT folio FROM notificaciones WHERE contador = '1' AND tipo = 'REACTIVACION' AND folio > '0' GROUP BY folio DESC";
                        $resultReact = mysqli_query($conexion, $sqlReact);
                        while ($verReact = mysqli_fetch_array($resultReact)) {
                            $datoReact = $verReact[0];

                            $sqlUsuario = "SELECT usuario FROM notificaciones WHERE contador = '1' AND (tipo = 'REACTIVACION' AND folio = '$datoReact') GROUP BY folio";
                            $resultUsuario = mysqli_query($conexion, $sqlUsuario);
                            while ($verUsuario = mysqli_fetch_array($resultUsuario)) {
                                $datoUsuario = $verUsuario[0];
                            }

                            $sqlFecha = "SELECT fecha FROM notificaciones WHERE contador = '1' AND (tipo = 'REACTIVACION' AND folio = '$datoReact')";
                            $resultFecha = mysqli_query($conexion, $sqlFecha);
                            while ($verFecha = mysqli_fetch_array($resultFecha)) {
                                $datoFecha = $verFecha[0];
                                $newDate = date("d-m-Y", strtotime($datoFecha));
            ...
            ?>
                ...
                <td align="center">
                    <small>
                        <b><?= $datoUsuario ?></b> reactivo el <b>folio <?= $datoReact ?></b> el día <b><?= $newDate ?></b>
                    </small>
                ...
                <!-- Detalles -->
                <td align="center">
                    <form class="" action="seguimiento_colab.php?id=<?php echo $datoReact; ?>" method="post">
