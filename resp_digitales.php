...
//Guarda la informacion atraves del requests que el usuario hace durante la visita al sitio
$nomusuario = $_SESSION['nomusuario'];

//Valida si existen estos dos usuarios para el acceso
...
?>
    ...
    <head>
        ...
        <!-- HOJAS DE ESTILO -->
        <link rel="stylesheet" href="css/hoja_respaldosDig.css">
        <!-- DATATABLES -->
        <script type="text/javascript" charset="utf8" src="https://cdn.datatables.net/1.10.24/js/jquery.dataTables.js"></script>
    </head>

    <?php
    // Cabecera para digitalizacion en pymes
    ...
    <body>
        ...
        <!-- TABLA / INFORMACION DE LOS ARCHIVOS -->
        <div class="container">
            <div class="row" style="width: 1390px;">
                <table id="example" class="table table-condensed table-bordered table-striped">
                    ...
                    <tbody>
                        <?php
                        $archivo = "SELECT * FROM carpetas_digitalizacion_pymes ";
                        $resultT = mysqli_query($conexion, $archivo);
                        while ($archivos = mysqli_fetch_array($resultT)) {
                        ?>
                            <tr>
                                <td style="font-weight: bold;"><?php echo $archivos['nombreContratante']; ?></td>
                                <td style="width: 170px;"><?php echo $archivos['agente']; ?></td>
                                <td><?php echo $archivos['numeroP']; ?></td>
                                <td><?php echo $newDate3 = date("d/m/Y", strtotime(utf8_decode($archivos['vigencia']))); ?></td>
                                <td>$<?php echo $archivos['primaNeta']; ?></td>
                                <td><?php echo $archivos['formaPago']; ?></td>
                                <td><?php echo $archivos['moneda']; ?></td>
                                ...
                                <td>
                                    <?php
                                    $sql1 = "SELECT COUNT(id_contratante) FROM archivos_digitalizacion_pymes WHERE id_contratante ='$archivos[id]'";
                                    $resultado1 = mysqli_query($conexion, $sql1);

                                    while ($row1 = $resultado1->fetch_assoc()) {
                                        echo $row1['COUNT(id_contratante)'];
                                    }
                                    ?>
                                </td>
                                <td>
                                    <form id="archivo" action="detalles_py.php" method="POST">
                                        ...
                                        <input style="display: none;" type="radio" name="agente" value="<?php echo $archivos['agente'];  ?>" checked>
                                        <input style="display: none;" type="radio" name="numeroP" value="<?php echo $archivos['numeroP'];  ?>" checked>
                                        <input style="display: none;" type="radio" name="vigencia" value="<?php echo $archivos['vigencia'];  ?>" checked>
                                        <input style="display: none;" type="radio" name="primaNeta" value="<?php echo $archivos['primaNeta'];  ?>" checked>
                                        <input style="display: none;" type="radio" name="formaPago" value="<?php echo $archivos['formaPago'];  ?>" checked>
                                        <input style="display: none;" type="radio" name="moneda" value="<?php echo $archivos['moneda'];  ?>" checked>
        ...
        <div class="modal fade" id="modalCarpeta" tabindex="-1" aria-hidden="true" data-backdrop="static" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <form method="POST" action="crearCarpetaPy.php" enctype="multipart/form-data">
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <textarea name="nombreContratante" class="number form-control input-sm" placeholder="Ingresa un nombre" onkeyup="mayusculas(this);"></textarea>

                            <label>Agente:</label>
                            <select class="form-control input-sm" id="agente" name="agente">
                            ...
                            <textarea name="numeroP" class="number form-control input-sm" placeholder="Ingresa la póliza correspondiente" onkeyup="mayusculas(this);"></textarea>

                            <label>Vigencia:</label>
                            <input class="form-control input-sm" type="date" id="vigencia" name="vigencia">

                            <label>Prima Neta:</label>
                            <input type="text" name="primaNeta" value="" id="primaNeta" class="number form-control input-sm" id="prima1" placeholder="Ingrese la prima correspondiente" maxlength="20" oninput="if(this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" />
                            ...
                            <label>Ramo:</label>
                            <select class="form-control input-sm" id="ramo" name="ramo">
                                <option selected disabled hidden>Seleccione: </option>
                                <option value="AUTOS">AUTOS</option>
                                <option value="AUTOS EQUIPO PESADO">AUTOS EQUIPO PESADO</option>
                                <option value="DAÑOS">DAÑOS</option>
                                <option value="GMM GRUPO">GMM GRUPO</option>
                                <option value="VIDA GRUPO">VIDA GRUPO</option>
                            </select>

                            <label>Comentario:</label>
                            <textarea name="comentario" class="comentarios form-control input-sm" placeholder="Ingresa un breve comentario (opcional)" onkeyup="mayusculas(this);"></textarea>
