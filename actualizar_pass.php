        ...
        <div class="row">
            <div class="col-sm-5 col-sm-offset-3">
                ...
                <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                    ...
                    <input type="password" class="form-control" name="contrasena_actual" id="contrasena_actual" placeholder="Ingresa tu contraseña actual" autocomplete="off" required>
                    ...
                    <button type="submit" class="btn-load btn-lg btn btn-primary col-xs-12" name="enviar" id="actualizarPass" disabled>Actualizar</button>
                </form>
            </div>
        </div>
    </div>
...
if (isset($_POST['enviar'])) {
    ...
    $actualizar = "UPDATE datos_agente SET password = '$contrasena_encryptada' WHERE nomusuario='$nomusuario'";

    $verificar = mysqli_query($conexion, "SELECT password FROM datos_agente WHERE password='$contrasena_actual';");
        ...
        if ($res) {
            ...
            jQuery(function () {
                swal({
                    ...
                    function () {
                        window.location.href = "menu_agente.php";
                    })
            });
            </script> ';
        } else {
            ...
            //echo '<script>alert("Problemas con el servidor, favor de intentar más tarde"); window.history.go(-1);</script>';
        }
    } else {
        echo '<script> 
        jQuery(function () {
            swal({
                ...
                function () {
                    window.location.href = "actualizar_pass.php";
                })
        });
        </script> ';
        //echo '<script>alert("La contraseña actual no coincide, favor de intentarlo nuevamente."); window.history.go(-1);</script>';
    }
}
?>