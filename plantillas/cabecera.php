    ...
    <nav class="navbar navbar-default" role="navigation">
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="myNavbar">
                ...
                <!-- Querys para contabilizar el total de notificaciones -->
                <ul class="nav navbar-nav navbar-right">
                    <?php
                    //Querys para validar cuantas notificaciones tiene cada seccion
                    $sqlC = "SELECT COUNT(DISTINCT folio) FROM notificaciones WHERE contador = '1' AND folio > '0' AND tipo = 'COMENTARIO'";
                    $resultC = mysqli_query($conexion, $sqlC);
                    while ($verC = mysqli_fetch_row($resultC)) {
                        $datosC = $verC[0];
                    }

                    $sqlA = "SELECT COUNT(DISTINCT folio) FROM notificaciones WHERE contador = '1' AND folio > '0' AND tipo = 'ARCHIVO'";
                    $resultA = mysqli_query($conexion, $sqlA);
                    while ($verA = mysqli_fetch_row($resultA)) {
                        $datosA = $verA[0];
                    }

                    $sqlT = "SELECT COUNT(DISTINCT folio) FROM notificaciones WHERE contador = '1' AND folio > '0' AND tipo = 'TRAMITE'";
                    $resultT = mysqli_query($conexion, $sqlT);
                    while ($verT = mysqli_fetch_row($resultT)) {
                        $datosT = $verT[0];
                    }

                    $sqlR = "SELECT COUNT(DISTINCT folio) FROM notificaciones WHERE contador = '1' AND folio > '0' AND tipo = 'REACTIVACION'";
                    ...
                    <li><a href="notificaciones.php" title="Notificaciones"><span class="glyphicon glyphicon-bell"></span>
                            <?php
                            echo $totalN;
                            ?>
                        </a>
                    ...
                    <li><a href="folios.php"><span class="glyphicon glyphicon-search" title="Búsqueda de Folios"></span></a></li>
                    <li><a href="activar.php" title="Activar Folios"><span class="glyphicon glyphicon-retweet"></span></a></li>
                    <!--ESTA LINEA DIRIGE A LA HOJA DE ACTIVAR FOLIOS EN VIDA QUE ESTA UBICADO EN RAIZ-->
                    <li><a href="menu_consultor.php"><span class="glyphicon glyphicon-menu-hamburger"></span> Menú</a></li>
                    <li><a href="consultor.php"><span class="glyphicon glyphicon-chevron-left"></span> Regresar</a></li>
